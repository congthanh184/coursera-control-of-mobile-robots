...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            if(strcmp(inputs.direction,'right'))
                ...
                ir_distances
%                 temp = ir_distances(5:-1:3);
                [~, id] = sort(ir_distances(5:-1:3));
                id = 6 - id;
                if (id(1) < id(2))
                    temp = id(1);
                    id(1) = id(2);
                ...
                p_1 = ir_distances_wf(:, id(1));
                p_2 = ir_distances_wf(:, id(2));
            else
                % Pick two of the left sensors based on ir_distances
%                 ir_distances
                [~, id] = sort(ir_distances(1:3));
%                 id
                if (id(1) > id(2))
                    temp = id(1);
                    id(1) = id(2);
...
%             u1 = (u_fw_p / norm(u_fw_p));
%             u2 = (u_fw_pp / norm(u_fw_pp));
%             u_fw = u_fw_tp;
%             alpha = 0.65;
%             u_fw = alpha * u_fw_tp + (1-alpha) * u2;
