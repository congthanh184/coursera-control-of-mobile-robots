    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            elseif (obj.check_event('at_obstacle'))
                ...
                if ((obj.is_in_state('go_to_goal') || obj.is_in_state('ao_and_gtg')) && ~obj.check_event('progress_made'))
                    if (obj.check_event('sliding_left'))
                        inputs.fw_direction = 'left';
                    else 
                        inputs.fw_direction = 'right';
                    end
            ...
            elseif (obj.check_event('obstacle_cleared'))
                if ~(obj.is_in_state('follow_wall') && ~obj.check_event('progress_made'))
...
%                 fprintf('now sliding right\n');
        ...
        function rc = progress_made(obj, state, robot)
            ...
            %% START CODE BLOCK %%
            distance_to_goal = [x;y] - obj.goal';
            epsilon = 0.1;

            if (norm(distance_to_goal) < obj.d_prog - epsilon)
                rc = true;
            end
